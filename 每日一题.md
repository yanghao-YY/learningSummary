...
/**
 * @Auther: licl
 * @Description: 二叉树前中后序遍历   非递归版本  递归其实就是利用系统栈，如果不用递归我们可以自己设计压栈操作来实现遍历, 不知道如何压栈，可以先理清楚递归版的过程，然后就会有思路
 * @Date: 2021/7/26
 * @Desc: @TODO
 */
public class BinaryTreeTraversal1 {
    ...
    /**
     * 前序遍历 头左右
     * 将将头节点放入栈，取时打印，然后判断该节点是否有左右子节点，有的话先放入右节点，再放入左节点
     * @param head
     */
    public static void pre(Node head){
        if(head != null){
    ...
    /**
     * 后序遍历 左右头 -> 逆序为头右左，所以与前序遍历类似，只是在放入节点的左右孩子节点时，先放入左节点，后放入右节点。然后用另一个栈来逆序 头右左 即可达到左右头
     * @param head
     */
    public static void pos(Node head){
        if(head != null){
